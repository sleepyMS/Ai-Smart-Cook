    ...
    return (
        <header id='header' role='banner'>
            ...
            <div className="rightButtons">
                ...
                {peopleName === "로그인" && <button onClick={() => navigate('/registerpage1')}>회원가입</button>}
                {peopleName !== "로그인" && <button onClick={handleLogout}>로그아웃</button>}
            </div>

            <div className='searchForm'>
                <form onSubmit={onSubmit}>
                    <input type='text' placeholder='재료 -> 음식 or 음식 -> 재료를 검색하시오.' value={searchInput} onChange={handleChange} />
